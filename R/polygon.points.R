...
polygon.points <- function(latitude, longitude, latitude_width, longitude_width){
  ifelse(is.null(latitude_width), latitude_width <- MASS::bandwidth.nrd(latitude), latitude_width)
  ifelse(is.null(longitude_width), longitude_width <- MASS::bandwidth.nrd(longitude), longitude_width)
  kde <- MASS::kde2d(longitude,
                     latitude,
                     n = 100,
                     h = c(longitude_width, latitude_width),
                     lims = c(min(stats::density(longitude)$x)-sd(longitude),
                              max(stats::density(longitude)$x)+sd(longitude),
                              min(stats::density(latitude)$x)-sd(latitude),
                              max(stats::density(latitude)$x)+sd(latitude)))

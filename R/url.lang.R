...
url.lang <- function(x, popup = "", glottolog.source = "modified") {
    ...
    df$link[!is.na(df$iso)] <- paste0(
      "<a href='", "http://glottolog.org/resource/languoid/iso/",
      df$iso[!is.na(df$iso)],
      "' target='_blank'>",
    ...
    df$link[is.na(df$iso) & !is.na(df$glottocode)] <- paste0(
      "<a href='", "http://glottolog.org/resource/languoid/id/",
      df$glottocode[is.na(df$iso) & !is.na(df$glottocode)],
      "' target='_blank'>",
      as.character(df$language[is.na(df$iso) & !is.na(df$glottocode)]),
      "</a><br>",

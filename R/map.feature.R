...
map.feature <- function(languages, features = "none", popup = "", stroke.features = NULL, 
    latitude = NULL, longitude = NULL, color = NULL, stroke.color = NULL, image.url = NULL, 
    image.width = 100, image.height = 100, image.X.shift = 0, image.Y.shift = 0, 
    title = NULL, stroke.title = NULL, control = FALSE, legend = TRUE, legend.opacity = 1, 
    stroke.legend = TRUE, stroke.legend.opacity = 1, radius = 5, stroke.radius = 9.5, 
    opacity = 1, stroke.opacity = 1, tile = "OpenStreetMap.Mapnik", tile.name = NULL, 
    glottolog.source = "modified") {
    
    ifelse(grepl(glottolog.source, "original"), glottolog <- lingtypology::glottolog.original, 
        glottolog <- lingtypology::glottolog.modified)
    if (sum(is.glottolog(languages, response = TRUE, glottolog.source = glottolog.source)) == 
        0) {
        stop("There is no data to map")
    }
    
    # creat dataframe --------------------------------------------------------- if
    # there are no latitude and longitude
    if (is.null(latitude) & is.null(longitude)) {
        mapfeat.df <- data.frame(languages, features, long = long.lang(languages, 
            glottolog.source = glottolog.source), lat = lat.lang(languages, glottolog.source = glottolog.source), 
            popup = popup)
    } else {
        # if there are latitude and longitude
        mapfeat.df <- data.frame(languages, features, long = longitude, lat = latitude, 
            popup = popup)
    }
    ...
    # creata a pallet ---------------------------------------------------------
    if (is.null(color)) {
        pal <- leaflet::colorFactor(sample(rainbow(length(unique(mapfeat.df$features))), 
            length(unique(mapfeat.df$features))), domain = mapfeat.df$features)
    } else {
    ...
    if (!is.null(stroke.features)) {
        if (is.null(stroke.color)) {
            ...
            stroke.pal <- leaflet::colorFactor(strokecolors, domain = mapfeat.stroke$stroke.features)
            rev.stroke.pal <- leaflet::colorFactor(rev(strokecolors), domain = mapfeat.stroke$stroke.features)
        } else {
            stroke.pal <- leaflet::colorFactor(stroke.color, domain = mapfeat.stroke$stroke.features)
            rev.stroke.pal <- leaflet::colorFactor(rev(stroke.color), domain = mapfeat.stroke$stroke.features)
        }
    }
    
    # change feature names ----------------------------------------------------
    levels(mapfeat.df$features) <- paste(names(table(mapfeat.df$features)), " (", 
        table(mapfeat.df$features), ")", sep = "")
    
    # change tile names if needed ---------------------------------------------
    ifelse(is.null(tile.name), tile.name <- tile, NA)
    if (length(tile.name) != length(tile)) {
        tile.name <- tile
        ifelse(length(tile.name) > length(tile), warning("number of tiles (tile argument) is less than number of tile names (tile.name argument)", 
            call. = FALSE), warning("number of tile names (tile.name argument) is less than number of tiles (tile argument)", 
            call. = FALSE))
    }
    ...
    if (length(table(mapfeat.df$features)) <= 1) {
        ...
        m <- leaflet::leaflet(mapfeat.df) %>% leaflet::addTiles(tile[1]) %>% leaflet::addProviderTiles(tile[1], 
            group = tile.name[1])
        if (length(tile) > 1) {
        ...
        m <- m %>% leaflet::addCircleMarkers(lng = mapfeat.df$long, lat = mapfeat.df$lat, 
            popup = mapfeat.df$link, stroke = FALSE, radius = radius, color = color, 
            fillOpacity = opacity, group = mapfeat.df$languages)
        
        if (length(tile) > 1) {
            if (control == TRUE) {
                m <- m %>% leaflet::addLayersControl(baseGroups = tile.name, overlayGroups = mapfeat.df$languages, 
                  options = layersControlOptions(collapsed = FALSE))
            } else {
                m <- m %>% leaflet::addLayersControl(baseGroups = tile.name, options = layersControlOptions(collapsed = FALSE))
            }
        } else {
            if (control == TRUE) {
        ...
        if (!is.null(image.url)) {
            m <- m %>% leaflet::addMarkers(lng = mapfeat.image$long, lat = mapfeat.image$lat, 
                popup = mapfeat.image$link, icon = icons(iconUrl = as.character(mapfeat.image$image.url), 
                  iconWidth = image.width, iconHeight = image.height, iconAnchorX = -image.X.shift, 
        ...
        m <- leaflet::leaflet(mapfeat.stroke) %>% leaflet::addTiles(tile[1]) %>% 
        ...
        m <- m %>% leaflet::addCircleMarkers(lng = mapfeat.stroke$long, lat = mapfeat.stroke$lat, 
            popup = mapfeat.stroke$link, stroke = FALSE, radius = stroke.radius * 
                1.15, fillOpacity = stroke.opacity, color = "black") %>% leaflet::addCircleMarkers(lng = mapfeat.stroke$long, 
            lat = mapfeat.stroke$lat, popup = mapfeat.stroke$link, stroke = FALSE, 
            radius = stroke.radius, fillOpacity = stroke.opacity, color = stroke.pal(mapfeat.stroke$stroke.features), 
            group = mapfeat.stroke$stroke.features) %>% leaflet::addCircleMarkers(lng = mapfeat.stroke$long, 
            lat = mapfeat.stroke$lat, popup = mapfeat.df$link, stroke = FALSE, radius = 1.15 * 
                radius, fillOpacity = opacity, color = rev.stroke.pal(mapfeat.stroke$stroke.features), 
            group = mapfeat.stroke$stroke.features) %>% leaflet::addCircleMarkers(lng = mapfeat.df$long, 
            lat = mapfeat.df$lat, popup = mapfeat.df$link, stroke = FALSE, radius = radius, 
            fillOpacity = opacity, color = pal(mapfeat.df$features), group = mapfeat.df$features)
        if (legend == TRUE) {
            m <- m %>% leaflet::addLegend(title = title, position = c("topright"), 
                pal = pal, values = mapfeat.df$features, opacity = legend.opacity)
        }
        if (stroke.legend == TRUE) {
            m <- m %>% leaflet::addLegend(title = stroke.title, position = c("bottomleft"), 
                pal = stroke.pal, values = mapfeat.stroke$stroke.features, opacity = stroke.legend.opacity)
        }
        if (!is.null(image.url)) {
            m <- m %>% leaflet::addMarkers(lng = mapfeat.image$long, lat = mapfeat.image$lat, 
                popup = mapfeat.image$link, icon = icons(iconUrl = as.character(mapfeat.image$image.url), 
        ...
        if (length(tile) > 1) {
            if (control == TRUE) {
                m <- m %>% leaflet::addLayersControl(baseGroups = tile.name, overlayGroups = mapfeat.df$features, 
                  options = layersControlOptions(collapsed = FALSE))
            } else {
                m <- m %>% leaflet::addLayersControl(baseGroups = tile.name, options = layersControlOptions(collapsed = FALSE))
            }
        ...
        m <- m %>% leaflet::addCircleMarkers(lng = mapfeat.df$long, lat = mapfeat.df$lat, 
            popup = mapfeat.df$link, stroke = FALSE, radius = radius, fillOpacity = opacity, 
            color = pal(mapfeat.df$features), group = mapfeat.df$features)
        ...
        if (legend == TRUE) {
            m <- m %>% leaflet::addLegend(title = title, position = c("bottomleft"), 
                pal = pal, values = mapfeat.df$features, opacity = opacity)
        }
    }
    m
}

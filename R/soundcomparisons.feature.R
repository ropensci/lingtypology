...
@inproceedings{heggarty_sound_2019,
    ...
    soundcomparisons <- lingtypology::soundcomparisons
    soundcomparisons$glottocode <- lingtypology::gltc.lang(c(soundcomparisons$LanguageName))
    thisisfinal_df <-  data.frame()
    for (x in word){
      if ((x %in% word_set)||(x %in% word_set_2)){
        x <- (words[words$WordModernName1 == x,])
        ...
        links_set <- c()
        langs <- paste(soundcomparisons$LanguageId, collapse=",")
        myword <- words[which(words$WordModernName1 == x), ]
        for (link in families) {
          links <- paste0("https://soundcomparisons.com/export/csv?study=",link,
                          "&languages=", langs,"&words=", word_id)
          links_set <- append(links_set, links)}

        df <- data.frame(matrix(ncol = 8, nrow = 0))
        x <- c("LanguageId", "LanguageName", "Latitude", "Longitude",
               "WordId", "WordModernName1", "WordProtoName1", "Phonetic")
        colnames(df) <- x
        for (link in links_set) {
          datalist  <-  read.csv(link, encoding = "UTF-8")
      ...
      final_df <- merge(df, soundcomparisons, by = "LanguageId")
      df_2 <- data.frame(matrix(ncol = 8, nrow = 0))
      df_2 <- rbind(df_2, final_df)
      df_2$language <- df_2$LanguageName.x
      df_2$latitude <- df_2$Latitude
      df_2$longitute <- df_2$Longitude
      df_2$word_id <- df_2$WordId
      df_2$modern_word <- df_2$WordModernName1
      df_2$proto_word <- df_2$WordProtoName1
      df_2$phonetic <- df_2$Phonetic
      df_2 <- subset(df_2, select = -c(LanguageId, LanguageName.x, LanguageName.y,
                                      Latitude, Longitude, WordId, WordModernName1,
                                     WordProtoName1, Phonetic ))
      thisisfinal_df <- rbind(thisisfinal_df, df_2)}
  return(thisisfinal_df)
  }

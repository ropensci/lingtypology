...
There are several types of variables in R and `map.feature` works differently depending on the variable type. I will use a build in data set `ejective_and_n_consonants` that contains 27 languages from [LAPSyD database](http://www.lapsyd.ddl.ish-lyon.cnrs.fr/lapsyd/). This dataset have two variables: the categorical variable `ejectives` indicates whether some language has any ejective sound, the numeric variable `consonants` contains information about the number of consonants (based on LAPSyD database). We can create two maps with categorical variable and with numeric variable:
...
Unfortunately, this argument limits work of the other arguments such as `label`, `popup` and others, but provides beutifull functionality of `leaflet.minicharts` library.

...
## 1. Installation
Since `lingtypology` is an R package, you should install [R (version >= 3.1.0)](https://www.r-project.org/) on your PC if you haven't already done so. To install the `lingtypology` package, run the following command at your R IDE, so you get a stable version from CRAN:
```{r, eval=FALSE}
install.packages("lingtypology")
```

You can also get a development version from GitHub:
```{r, eval= F}
...
There are some functions that take country names as input. Unfortunately, some countries have alternative names. In order to save users from the trouble of having to figure out the exact name stored in the database (for example  _Ivory Coast_ or _Cote d'Ivoire_), all official country names and standard abbreviations are stored in the database:
...
There are several types of variables in R and `map.feature` works differently depending on the variable type. I will use a build in the data set `ejective_and_n_consonants` that contains 27 languages from [LAPSyD database](http://www.lapsyd.ddl.ish-lyon.cnrs.fr/lapsyd/). This dataset have two variables: the categorical variable `ejectives` indicates whether some language has any ejective sound, the numeric variable `n.cons.lapsyd` contains information about the number of consonants (based on LAPSyD database). We can create two maps with categorical variable and with numeric variable:
...
There are some builts in packages that also can be used as a color argument: [RColorBrewer](https://CRAN.R-project.org/package=RColorBrewer) or [viridis](https://CRAN.R-project.org/package=viridis).
...
Using this argument, users can plot their own markers, any chart connected to a point or even their own legend. It is important to know that by using transparent .png files, a user can plot an additional legend text on the map.
...
### 4.6 ABVD
The ABVD database is a lexical database, so it is different from other clld databases. First of all, ABVD has its own language classification ids. The information about the same language from different sources can be received from these database different ids. So I select several languages and map them coloring with the word that means 'hand'.
```{r, fig.width=6.2, fig.height=3.4}
...
## 5. `dplyr` integration
It is possible to use `dplyr` functions and pipes with `lingtypology`. It is widely used, so I will give some examples, how to use it with the`lingtypology` package. Using query "list of languages csv" I found Vincent Garnier's [languages-list repository](https://github.com/forxer/languages-list). Let’s download and map all the languages from that set. Let’s download the data:

...
```
I would like to mention that strings in R can be created using
single or double quotes. Since inserting single quotes in a string created with single quotes causes a mistake in R, I use double quotes in my tutorial. You can use single quotes, but be careful and remember that `'Ma'ya'` is wrong string in R.
...
* Country name Viet Nam changed to Vietnam. Madagascar and Togo to countries set added. (11.02.2017)
...
If for some reasons you are not using RStudio or you want automatically create and save a lot of maps, you can save a map to the variable and use `htmlwidgets` package for saving created maps to an .html file. I would like to thank Timo Roettger for mentioning that problem.
...
I've got an export button in RStudio, but for some reason it is not so easy to save map as .png or .jpg file. [Here](http://stackoverflow.com/a/34672309/6056442) is some solution.
...
If you have a lot of features and they are appear in the legend in senseless order (by default it is alphabatical order), you can reorder them using factors (vector with ordered levels, for more information see `?factor`). For example, I want polysynthetic feature be first and fusional feature be last:
...
### 3.4 Set labels
An alternative way to add some short text to map is to use `label` option.
```{r, fig.width=6.2}
set.seed(42)
map.feature(df$language, df$features,
            label = df$language)
...
There are some additional arguments for customization: `label.fsize` for setting font size, `label.position` for controlling label position, and `label.hide` that gives user a control on label appearence: if `TRUE` than labels are displayed on mouse over (as on previous map), if `FALSE` than labels are displayed always (as on the next map).
...
```
There are arguments `legend.position` and `stroke.legend.position` that allow users to change legends position using "topright", "bottomright", "bottomleft" or "topleft" strings.

### 3.11 Set scale bar
Scale bar is automatically added to a map, but users can control its appearance (set `scale.bar` argument to TRUE or FALSE) and its position (use `scale.bar.position` argument values "topright", "bottomright", "bottomleft" or "topleft").
```{r, fig.width=6.2}
set.seed(42)
...
Users can control its appearance (set `minimap` argument to TRUE or FALSE), its position (use `minimap.position` argument values "topright", "bottomright", "bottomleft" or "topleft"), its height and width (arguments `minimap.height` and `minimap.width`).
```{r, fig.width=6.2}
set.seed(42)
map.feature(c("Adyghe", "Polish", "Kabardian", "Russian"),
            minimap = TRUE,

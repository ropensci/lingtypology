...
After [Robert Forkel's issue](https://github.com/agricolamz/lingtypology/issues/1) I decided to add an argument `glottolog.source`, so that everybody have an access to "original" (by default) and "modified" glottolog versions:
```{r}
...
It is common practice in R to reduce both function arguments and its values. So it could be done with `lingtypology` functions either.
```{r}
...
### 3.10 Set layouts
It is possible to use different map tiles  using `tile` argument. For more tiles see [here](https://leaflet-extras.github.io/leaflet-providers/preview/index.html).
```{r, fig.width=6.2}
...
set.seed(11)
map.feature(df$lang, df$feature, df$popup,
                        tile = c("OpenStreetMap.BlackAndWhite", "Stamen.Toner"))
```

It is possible to combine different tiles and control box.
```{r, fig.width=6.2}
df <- data.frame(lang = c("Adyghe", "Kabardian", "Polish", "Russian", "Bulgarian"),
   feature = c("polysynthetic", "polysynthetic", "fusion", "fusion", "fusion"),
   popup = c("Adyghe", "Adyghe", "Slavic", "Slavic", "Slavic"))
...
Argument `images.url` allows users to add their own pictures to the map, using url. In this part I will use two histograms on most numerous nations in Moscow and St. Petersburg based on last Russian Census data:
...
Using this argument users can plot their own markers, any chart connected to the point or even their own legend. It is important to know, that using transparent .png files, user can can plot additional legend text on the map.

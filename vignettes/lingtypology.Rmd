...
* Abkhazian is changed to Abkhaz. Abkhazian, Abaza, Georgian, Ossetian and Ingush coordinates are changed. (22.01.2017)
...
Default colors are not perfect for this goal, but the main point is clear. For creating correct map, you should correctly define the tipe of the variable.

This dataset also can be used to show one other parameter of the `map.feature` function. There are two possible ways to show the World map: with the Atlantic sea or with the Pacific sea in the middle. If you don't need default Pacific view use the `map.orientation` parameter:
```{r, fig.width=6.2, fig.height=3.4}
map.feature(ejective_and_n_consonants$language,
            ejective_and_n_consonants$n.cons.lapsyd,

library(lingtypology); library(magrittr); library(leaflet)
...
test_that("map.feature source", {
  expect_equal(exists("map"),
               TRUE)
  expect_equal(exists("map2"),
               TRUE)
  expect_warning(map.feature(c("Tabassaran", "Adyghe")),
                 "Languoid Tabassaran is absent in our database. Did you mean Tabasaran?")
  expect_warning(map.feature(c("Tabasaran", "Adyghe"), glottolog.source = "o"),
...
test_that("map.feature coordinates", {
  expect_equal(map$x$limits[[1]][1],
               unname(lat.lang("Tabasaran")))
  expect_equal(map$x$limits[[2]][1],
               unname(long.lang("Tabasaran")))
  expect_equal(map_coord$x$limits[[1]][1],
               43)
  expect_equal(map_coord$x$limits[[2]][1],
               57)
})

map_image <- map.feature("Tabasaran",
                         image.url = "https://goo.gl/Ycn6tJ")
...
test_that("map.feature colors", {
  expect_equal(map_colors$x$calls[[3]]$args[6][[1]]$color,
               c("navy", "yellowgreen"))
  expect_equal(map_colorless$x$calls[[3]]$args[6][[1]]$color,
               "blue")
  expect_equal(map_stroke$x$calls[[3]]$args[6][[1]]$color,
               "black")
})

map_tiles <- map.feature("Adyghe",
                         tile = c("OpenStreetMap.BlackAndWhite", "Thunderforest.OpenCycleMap"))

test_that("map.feature tiles", {
  expect_equal(c(map_tiles$x$calls[[2]]$args[[1]],
                 map_tiles$x$calls[[3]]$args[[1]]),

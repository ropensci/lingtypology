...
test_that("map.feature stroke feature", {
    expect_equal(length(map_stroke$x$calls), 8)
    expect_equal(map_stroke2$x$calls[[4]]$args[[6]]$fillColor, c("#0000FF", "#0000FF", "#00FF00", 
        "#00FF00", "#00FF00"))
})

map_colorless <- map.feature(c("Tabasaran", "Adyghe"))
map_colors <- map.feature(c("Tabasaran", "Adyghe"), color = c("navy", "yellowgreen"))

test_that("map.feature colors", {
    expect_equal(map_colors$x$calls[[3]]$args[6][[1]]$color, c("navy", "yellowgreen"))
